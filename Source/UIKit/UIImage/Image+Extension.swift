...
public extension UIImage{
    // MARK: - 降低质量
    func resetSize(maxSize: Int) -> NSData? {
        
        //先判断当前质量是否满足要求，不满足再进行压缩
        guard  var finallImageData = self.jpegData(compressionQuality: 1) else {
            return nil
        }
        ...
        guard let newImageData = newImage.jpegData(compressionQuality: 1) else {
            return nil
        }
        ...
        while finallImageData.count == 0 {
            ...
            let sourceImage = UIImage.init(data:newImage.jpegData(compressionQuality: compressionQualityArr.lastObject as! CGFloat)!)
            
            let image = self.newSizeImage(size: defaultSize,
                                          sourceImage: sourceImage!)
            
            finallImageData = self.halfFuntion(arr: compressionQualityArr.copy() as! [CGFloat], image: image, sourceData: image.jpegData(compressionQuality: 1)!, maxSize: maxSize)!
        }
